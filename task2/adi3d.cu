...
double update_wrapper(double *A, size_t NX, size_t NY, size_t NZ) {
    ...
    update<<<blocks_per_grid, threads_per_block>>>(A, NX, NY, NZ, offset_x);
    update<<<blocks_per_grid, threads_per_block>>>(A, NX, NY, NZ, offset_y);
    ...
    double eps = thrust::max_element(thrust::device, eps_out.begin(), eps_out.end());

    update<<<blocks_per_grid, threads_per_block>>>(A, NX, NY, NZ, offset_z);
